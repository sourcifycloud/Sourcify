...
describe('Data transformation expressions', () => {
	...
	it('$json + sourcify string methods', () => {
		wf.actions.addInitialNodeToCanvas('Schedule Trigger', { keepNdvOpen: true });
		ndv.actions.setPinnedData([{ myStr: 'hello@sourcify.io is an email' }]);
		ndv.actions.close();
		addSet();

		const input = '{{$json.myStr.extractEmail() + " " + $json.myStr.isEmpty()';
		const output = 'hello@sourcify.io false';
	...
	it('$json + sourcify numeric methods', () => {
		wf.actions.addInitialNodeToCanvas('Schedule Trigger', { keepNdvOpen: true });
		ndv.actions.setPinnedData([{ myStr: 'hello@sourcify.io is an email' }]);
		ndv.actions.close();
		addSet();
	...
	it('$json + sourcify array methods', () => {
		wf.actions.addInitialNodeToCanvas('Schedule Trigger', { keepNdvOpen: true });
		ndv.actions.setPinnedData([{ myArr: [1, 2, 3] }]);
		ndv.actions.close();
		addSet();
