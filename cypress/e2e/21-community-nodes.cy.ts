...
import CustomNodeWithsourcifyCredentialFixture from '../fixtures/Custom_node_sourcify_credential.json';
...
describe('Community Nodes', () => {
	...
	beforeEach(() => {
		cy.intercept('/types/nodes.json', { middleware: true }, (req) => {
			...
			req.on('response', (res) => {
				...
				nodes.push(CustomNodeFixture, CustomNodeWithsourcifyCredentialFixture, CustomNodeWithCustomCredentialFixture);
			});
		})
	...
	it('should render custom node with sourcify credential', () => {
		workflowPage.actions.addNodeToCanvas('Manual');
		workflowPage.actions.addNodeToCanvas('E2E Node with native sourcify credential', true, true);
