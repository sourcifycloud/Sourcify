 ...
 * eslint-plugin-sourcify-local-rules and exposed as 'sourcify-local-rules/<rule-name>'.
 */
module.exports = {
	/**
	 ...
	 * (abundant in the sourcify codebase):
	 *
	 * Valid:
	 * ```js
	...
	'no-uncaught-json-parse': {
		meta: {
			...
			docs: {
				description:
					'Calls to `JSON.parse()` must be replaced with `jsonParse()` from `sourcify-workflow` or surrounded with a try/catch block.',
				recommended: 'error',
			},
			schema: [],
			messages: {
				noUncaughtJsonParse:
					'Use `jsonParse()` from `sourcify-workflow` or surround the `JSON.parse()` call with a try/catch block.',
			},
		},
		defaultOptions: [],
		create(context) {
	...
	'no-json-parse-json-stringify': {
		meta: {
			...
			docs: {
				description:
					'Calls to `JSON.parse(JSON.stringify(arg))` must be replaced with `deepCopy(arg)` from `sourcify-workflow`.',
				recommended: 'error',
			},
			messages: {
