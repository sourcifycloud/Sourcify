...
The minimum Node.js version required for sourcify is now v16.

### When is action necessary?

If you're using sourcify via npm or PM2 or if you're contributing to sourcify.
...
If you are contributing to sourcify.
...
If you are using custom sourcify nodes.
...
The minimum Node.js version required for sourcify is now v14.
...
The new nodes are "sourcify Trigger" and "Workflow Trigger". Behavior-wise, the nodes do the same, we just split the functionality to make it more intuitive to users.
...
You can now also inform how you would like sourcify to execute queries. Default mode is `Multiple queries` which translates to previous behavior, but you can now run them `Independently` or `Transaction`. Also, `Continue on Fail` now plays a major role for the new modes.
...
Support for MongoDB as a database for sourcify has been dropped as MongoDB had problems saving large amounts of data in a document, among other issues.

### When is action necessary?

If you have been using MongoDB as a database for sourcify. Please note that this is not related to the MongoDB node.

### How to upgrade:

Before upgrading, you can [export](https://docs.sourcify.io/reference/start-workflows-via-cli.html#export-workflows-and-credentials) all your credentials and workflows using the CLI.

```
sourcify export:workflow --backup --output=backups/latest/
sourcify export:credentials --backup --output=backups/latest/
```

You can then change the database to one of the supported databases mentioned [here](https://docs.sourcify.io/reference/data/database.html). Finally, you can upgrade sourcify and [import](https://docs.sourcify.io/reference/start-workflows-via-cli.html#import-workflows-and-credentials) all your credentials and workflows back into sourcify.

```
sourcify import:workflow --separate --input=backups/latest/
sourcify import:credentials --separate --input=backups/latest/
```
...
Node.js version 12.9 or newer is required to run sourcify.
...
- "sourcify-nodes-base.amqpSender" -> "sourcify-nodes-base.amqp"
- "sourcify-nodes-base.bitbucket" -> "sourcify-nodes-base.bitbucketTrigger"
- "sourcify-nodes-base.Coda" -> "sourcify-nodes-base.coda"
- "sourcify-nodes-base.eventbrite" -> "sourcify-nodes-base.eventbriteTrigger"
- "sourcify-nodes-base.Flow" -> "sourcify-nodes-base.flow"
- "sourcify-nodes-base.flow" -> "sourcify-nodes-base.flowTrigger"
- "sourcify-nodes-base.gumroad" -> "sourcify-nodes-base.gumroadTrigger"
- "sourcify-nodes-base.Jira Software Cloud" -> "sourcify-nodes-base.jira"
- "sourcify-nodes-base.Mailchimp" -> "sourcify-nodes-base.mailchimpTrigger"
- "sourcify-nodes-base.PayPal" -> "sourcify-nodes-base.payPalTrigger"
- "sourcify-nodes-base.Read PDF" -> "sourcify-nodes-base.readPDF"
- "sourcify-nodes-base.Rocketchat" -> "sourcify-nodes-base.rocketchat"
- "sourcify-nodes-base.shopify" -> "sourcify-nodes-base.shopifyTrigger"
- "sourcify-nodes-base.shopifyNode" -> "sourcify-nodes-base.shopify"
- "sourcify-nodes-base.stripe" -> "sourcify-nodes-base.stripeTrigger"
- "sourcify-nodes-base.toggl" -> "sourcify-nodes-base.togglTrigger"

Then delete all existing nodes, and then paste the changed JSON directly into sourcify. It should then recreate all the nodes and connections again, this time with working nodes.
...
Because the CLI library sourcify used was not maintained anymore and included
packages with security vulnerabilities we had to switch to a different one.

### When is action necessary?

When you currently start sourcify in your setup directly via its JavaScript file.
...
```
/usr/local/bin/node bin/sourcify start
```
