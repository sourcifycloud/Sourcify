...
import type { WebhookHttpMethod } from 'sourcify-workflow';
import { ErrorReporterProxy as ErrorReporter, LoggerProxy as Logger } from 'sourcify-workflow';
import config from '@/config';
import { sourcify_VERSION, inDevelopment } from '@/constants';
	...
	async start(): Promise<void> {
		...
		this.server.on('error', (error: Error & { code: string }) => {
			if (error.code === 'EADDRINUSE') {
				console.log(
					`sourcify's port ${PORT} is already in use. Do you have another instance of sourcify running already?`,
				);
				process.exit(1);
			}
		});
		...
		console.log(`sourcify ready on ${ADDRESS}, port ${PORT}`);
		console.log(`Version: ${sourcify_VERSION}`);
		...
		await externalHooks.run('sourcify.ready', [this, config]);
	}
}
