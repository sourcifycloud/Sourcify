...
import { ActiveWorkflows, NodeExecuteFunctions } from 'sourcify-core';
...
export class ActiveWorkflowRunner {
	...
	async init() {
		...
		if (!config.getEnv('endpoints.skipWebhooksDeregistrationOnShutdown')) {
			...
			// This flag is set when sourcify is running in scaled mode.
			// Impact is minimal, but for a short while, sourcify will stop accepting requests.
		...
		for (const webhookData of webhooks) {
			...
			try {
				...
				if (
					...
					// sourcify does not remove the registered webhooks on exit.
	...
	/**
	 * Return poll function which gets the global functions from sourcify-core
	...
	/**
	 * Return trigger function which gets the global functions from sourcify-core
	 * and overwrites the emit to be able to start it in subprocess
	 *
	 */
