...
import type { PublicInstalledPackage } from 'sourcify-workflow';
import { LoggerProxy } from 'sourcify-workflow';
...
export const executeCommand = async (
	...
	const downloadFolder = UserSettings.getUsersourcifyFolderDownloadedNodesPath();
	...
	try {
		...
		Object.entries(map).forEach(([npmMessage, sourcifyMessage]) => {
			if (errorMessage.includes(npmMessage)) throw new Error(sourcifyMessage);
		});
...
export async function checkNpmPackageStatus(
	...
	const sourcify_BACKEND_SERVICE_URL = 'https://api.sourcify.io/api/package';

	try {
		const response = await axios.post<CommunityPackages.PackageStatusCheck>(
			sourcify_BACKEND_SERVICE_URL,
			{ name: packageName },
			{ method: 'POST' },
		);
