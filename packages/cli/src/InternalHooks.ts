...
import { BinaryDataManager } from 'sourcify-core';
import type {
	ExecutionStatus,
	INodesGraphResult,
	IRun,
...
import { TelemetryHelpers } from 'sourcify-workflow';
...
export class InternalHooks implements IInternalHooksClass {
	...
	async onServerStarted(
		...
		const info = {
			...
			sourcify_version_notifications_enabled: diagnosticInfo.notificationsEnabled,
			sourcify_disable_production_main_process: diagnosticInfo.disableProductionWebhooksOnMainProcess,
			sourcify_basic_auth_active: diagnosticInfo.basicAuthActive,
			system_info: diagnosticInfo.systemInfo,
			execution_variables: diagnosticInfo.executionVariables,
			sourcify_deployment_type: diagnosticInfo.deploymentType,
			sourcify_binary_data_mode: diagnosticInfo.binaryDataMode,
			sourcify_multi_user_allowed: diagnosticInfo.sourcify_multi_user_allowed,
	...
	async onWorkflowCreated(user: User, workflow: IWorkflowBase, publicApi: boolean): Promise<void> {
		...
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.workflow.created',
				payload: {
					...userToPayload(user),
	...
	async onWorkflowDeleted(user: User, workflowId: string, publicApi: boolean): Promise<void> {
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.workflow.deleted',
				payload: {
					...userToPayload(user),
					workflowId,
				},
	...
	async onWorkflowSaved(user: User, workflow: IWorkflowDb, publicApi: boolean): Promise<void> {
		...
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.workflow.updated',
				payload: {
					...userToPayload(user),
					workflowId: workflow.id,
					workflowName: workflow.name,
	...
	async onNodeBeforeExecute(
		...
		void eventBus.sendNodeEvent({
			eventName: 'sourcify.node.started',
			payload: {
				executionId,
				nodeName,
	...
	async onNodePostExecute(
		...
		const nodeInWorkflow = workflow.nodes.find((node) => node.name === nodeName);
		void eventBus.sendNodeEvent({
			eventName: 'sourcify.node.finished',
			payload: {
				executionId,
				nodeName,
				workflowId: workflow.id?.toString(),
	...
	async onWorkflowBeforeExecute(
		...
		void Promise.all([
			...
			eventBus.sendWorkflowEvent({
				eventName: 'sourcify.workflow.started',
				payload: {
					executionId,
					userId: data.userId,
	...
	async onWorkflowCrashed(
		...
		void Promise.all([
			eventBus.sendWorkflowEvent({
				eventName: 'sourcify.workflow.crashed',
				payload: {
					executionId,
	...
	async onWorkflowPostExecute(
		...
		promises.push(
			properties.success
				? eventBus.sendWorkflowEvent({
						eventName: 'sourcify.workflow.success',
						payload: {
							executionId,
				...
				: eventBus.sendWorkflowEvent({
						eventName: 'sourcify.workflow.failed',
						payload: {
							executionId,
							success: properties.success,
							userId: properties.user_id,
	...
	async onsourcifyStop(): Promise<void> {
		...
		return Promise.race([timeoutPromise, this.telemetry.tracksourcifyStop()]);
	}
	...
	async onUserDeletion(userDeletionData: {
		...
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.user.deleted',
				payload: {
	...
	async onUserInvite(userInviteData: {
		...
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.user.invited',
				payload: {
	...
	async onUserReinvite(userReinviteData: {
		...
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.user.reinvited',
				payload: {
	...
	async onUserUpdate(userUpdateData: { user: User; fields_changed: string[] }): Promise<void> {
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.user.updated',
				payload: {
	...
	async onUserInviteEmailClick(userInviteClickData: {
		...
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.user.invitation.accepted',
				payload: {
					invitee: {
	...
	async onUserPasswordResetEmailClick(userPasswordResetData: { user: User }): Promise<void> {
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.user.reset',
				payload: {
	...
	async onApiKeyDeleted(apiKeyDeletedData: { user: User; public_api: boolean }): Promise<void> {
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.user.api.deleted',
				payload: {
	...
	async onApiKeyCreated(apiKeyCreatedData: { user: User; public_api: boolean }): Promise<void> {
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.user.api.created',
				payload: {
	...
	async onUserPasswordResetRequestClick(userPasswordResetData: { user: User }): Promise<void> {
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.user.reset.requested',
				payload: {
					...userToPayload(userPasswordResetData.user),
				},
			}),
	...
	async onUserSignup(
		...
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.user.signedup',
				payload: {
					...userToPayload(user),
				},
			}),
	...
	async onEmailFailed(failedEmailData: {
		...
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.user.email.failed',
				payload: {
	...
	async onUserCreatedCredentials(userCreatedCredentialsData: {
		...
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.user.credentials.created',
				payload: {
	...
	async onUserSharedCredentials(userSharedCredentialsData: {
		...
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.user.credentials.shared',
				payload: {
	...
	async onCommunityPackageInstallFinished(installationData: {
		...
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.package.installed',
				payload: {
	...
	async onCommunityPackageUpdateFinished(updateData: {
		...
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.package.updated',
				payload: {
	...
	async onCommunityPackageDeleteFinished(deleteData: {
		...
		void Promise.all([
			eventBus.sendAuditEvent({
				eventName: 'sourcify.audit.package.deleted',
				payload: {
