...
import type { DirectoryLoader, Types } from 'sourcify-core';
import {
...
import { LoggerProxy, ErrorReporterProxy as ErrorReporter } from 'sourcify-workflow';
...
export class LoadNodesAndCredentials implements INodesAndCredentials {
	...
	async init() {
		...
		this.downloadFolder = UserSettings.getUsersourcifyFolderDownloadedNodesPath();

		// Load nodes from `sourcify-nodes-base` and any other `sourcify-nodes-*` package in the main `node_modules`
		const pathsToScan = [
			// In case "sourcify" package is in same node_modules folder.
			path.join(CLI_DIR, '..'),
			// In case "sourcify" package is the root and the packages are
	...
	private async loadNodesFromNodeModules(nodeModulesDir: string): Promise<void> {
		...
		const installedPackagePaths = [
			...(await glob('sourcify-nodes-*', { ...globOptions, deep: 1 })),
			...(await glob('@*/sourcify-nodes-*', { ...globOptions, deep: 2 })),
		];
	...
	getCustomDirectories(): string[] {
		const customDirectories = [UserSettings.getUsersourcifyFolderCustomExtensionPath()];
