import { UserSettings, Credentials } from 'sourcify-core';
import type { IDataObject, INodeProperties, INodePropertyOptions } from 'sourcify-workflow';
...
export async function createCredential(
	...
	if (!newCredential.nodesAccess || newCredential.nodesAccess.length === 0) {
		newCredential.nodesAccess = [
			{
				nodeType: `sourcify-nodes-base.${properties.type?.toLowerCase() ?? 'unknown'}`,
				date: new Date(),
			},
		];
	} else {
