...
/**
 * Return the sourcify instance base URL without trailing slash.
 */
export function getInstanceBaseUrl(): string {
	const sourcifyBaseUrl = config.getEnv('editorBaseUrl') || getWebhookBaseUrl();

	return sourcifyBaseUrl.endsWith('/') ? sourcifyBaseUrl.slice(0, sourcifyBaseUrl.length - 1) : sourcifyBaseUrl;
}
