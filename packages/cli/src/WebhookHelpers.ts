...
import { BinaryDataManager, NodeExecuteFunctions, eventEmitter } from 'sourcify-core';
...
export function decodeWebhookResponse(
	...
	if (
		...
		(response.body as IDataObject)['__@sourcifyEncodedBuffer@__']
	) {
		response.body = Buffer.from(
			(response.body as IDataObject)['__@sourcifyEncodedBuffer@__'] as string,
			BINARY_ENCODING,
		);
	}
...
export function encodeWebhookResponse(
	...
	if (typeof response === 'object' && Buffer.isBuffer(response.body)) {
		response.body = {
			...
			'__@sourcifyEncodedBuffer@__': response.body.toString(BINARY_ENCODING),
		};
	}
...
export async function executeWebhook(
	...
	try {
		...
		let responsePromise: IDeferredPromise<IsourcifyHttpFullResponse> | undefined;
		if (responseMode === 'responseNode') {
			responsePromise = await createDeferredPromise<IsourcifyHttpFullResponse>();
			responsePromise
				.promise()
				.then((response: IsourcifyHttpFullResponse) => {
					if (didSendResponse) {
						return;
					}
