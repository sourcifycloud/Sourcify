...
import { getsourcifyPackageJson, inDevelopment } from '@/constants';
import type { WorkflowEntity } from '@db/entities/WorkflowEntity';
import type { Risk, sourcify } from '@/audit/types';
...
function getSecuritySettings() {
	...
	if (isInstancePubliclyAccessible) {
		settings.publiclyAccessibleInstance =
			'Important! Your sourcify instance is publicly accessible. Any third party who knows your instance URL can access your data.'.toUpperCase();
	}
...
async function getNextVersions(currentVersionName: string) {
	...
	const response = await axios.get<sourcify.Version[]>(BASE_URL + currentVersionName, {
		// eslint-disable-next-line @typescript-eslint/naming-convention
		headers: { 'sourcify-instance-id': instanceId },
	});

	return response.data;
}

function removeIconData(versions: sourcify.Version[]) {
...
function classify(versions: sourcify.Version[], currentVersionName: string) {
...
export async function getOutdatedState() {
	...
	const localVersion = getsourcifyPackageJson().version;

	try {
		versions = await getNextVersions(localVersion).then(removeIconData);
	} catch (error) {
		if (inDevelopment) {
			console.error('Failed to fetch sourcify versions. Skipping outdated instance report...');
		}
		return null;
	}
	...
	const description = [
		`This sourcify instance is outdated. Currently at version ${
			currentVersion.name
...
export async function reportInstanceRisk(workflows: WorkflowEntity[]) {
	...
	if (outdatedState !== null) {
		report.sections.push({
			...
			recommendation:
				'Consider updating this sourcify instance to the latest version to prevent security vulnerabilities.',
	...
	if (securitySettings !== null) {
		report.sections.push({
			...
			description: 'This sourcify instance has the following security settings.',
			recommendation: securitySettings.publiclyAccessibleInstance
				? [
						'Important! Your sourcify instance is publicly accessible. Set up user management or basic/JWT auth to protect access to your sourcify instance.'.toUpperCase(),
						`See: ${SELF_HOSTED_AUTH_DOCS_URL}`,
				  ].join(' ')
				: `Consider adjusting the security settings for your sourcify instance based on your needs. See: ${ENV_VARS_DOCS_URL}`,
