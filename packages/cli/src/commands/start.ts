...
import { TUNNEL_SUBDOMAIN_ENV, UserSettings } from 'sourcify-core';
...
import { LoggerProxy, sleep, jsonParse } from 'sourcify-workflow';
...
export class Start extends BaseCommand {
	static description = 'Starts sourcify. Makes Web-UI available and starts active workflows';

	static examples = [
		'$ sourcify start',
		'$ sourcify start --tunnel',
		'$ sourcify start -o',
		'$ sourcify start --tunnel -o',
	];
	...
	static flags = {
		...
		tunnel: flags.boolean({
			description:
				'runs the webhooks via a hooks.sourcify.cloud tunnel server. Use only for testing and development!',
		}),
		reinstallMissingPackages: flags.boolean({
			description:
				'Attempts to self heal sourcify if packages with nodes are missing. Might drastically increase startup times.',
		}),
	};
	...
	/**
	 * Stop sourcify in a graceful way.
	...
	async stopProcess() {
		...
		try {
			...
			await this.externalHooks.run('sourcify.stop', []);
			...
			await Container.get(InternalHooks).onsourcifyStop();
			...
			await this.exitWithCrash('There was an error shutting down sourcify.', error);
		}
	...
	private async generateStaticAssets() {
		...
		const sourcifyPath = config.getEnv('path');
		...
		const compileFile = async (fileName: string) => {
			...
			if (/(index\.html)|.*\.(js|css)/.test(filePath) && existsSync(filePath)) {
				...
				const streams = [
					...
					replaceStream('/{{BASE_PATH}}/', sourcifyPath, { ignoreCase: false }),
					replaceStream('/%7B%7BBASE_PATH%7D%7D/', sourcifyPath, { ignoreCase: false }),
					replaceStream('/static/', sourcifyPath + 'static/', { ignoreCase: false }),
				];
	...
	async init() {
		...
		await super.init();
		this.logger.info('Initializing sourcify process');
	...
	async run() {
		...
		if (missingPackages.size) {
			LoggerProxy.error(
				'sourcify detected that some packages are missing. For more information, visit https://docs.sourcify.io/integrations/community-nodes/troubleshooting/',
			);

			if (flags.reinstallMissingPackages || process.env.sourcify_REINSTALL_MISSING_PACKAGES) {
				...
				try {
					...
					LoggerProxy.error('sourcify was unable to install the missing packages.');
				}
			}
		...
		if (flags.tunnel) {
			...
			const tunnelSettings: localtunnel.TunnelConfig = {
				host: 'https://hooks.sourcify.cloud',
			...
			this.log(
				'IMPORTANT! Do not share with anybody as it would give people access to your sourcify instance!',
			);
		}
		...
		// Allow to open sourcify editor by pressing "o"
