...
import type { ExecutionStatus, IExecuteResponsePromiseData, INodeTypes, IRun } from 'sourcify-workflow';
import { Workflow, NodeOperationError, LoggerProxy, sleep } from 'sourcify-workflow';
...
export class Worker extends BaseCommand {
	static description = '\nStarts a sourcify worker';

	static examples = ['$ sourcify worker --concurrency=5'];
	 ...
	 */
	async stopProcess() {
		LoggerProxy.info('Stopping sourcify...');
	...
	async init() {
		...
		await super.init();
		this.logger.debug('Starting sourcify worker...');
	...
	async run() {
		...
		this.logger.info('\nsourcify worker is now ready');
		this.logger.info(` * Version: ${sourcify_VERSION}`);
		...
		if (config.getEnv('queue.health.active')) {
			...
			server.listen(port, () => {
				this.logger.info(`\nsourcify worker health check via, port ${port}`);
			});

			server.on('error', (error: Error & { code: string }) => {
				if (error.code === 'EADDRINUSE') {
					this.logger.error(
						`sourcify's port ${port} is already in use. Do you have the sourcify main process running on that port?`,
					);
					process.exit(1);
				}
			});
