...
import type { sourcify } from 'sourcify-core';
import { RESPONSE_ERROR_MESSAGES as CORE_RESPONSE_ERROR_MESSAGES, UserSettings } from 'sourcify-core';
...
export const NODES_BASE_DIR = dirname(require.resolve('sourcify-nodes-base'));
export const GENERATED_STATIC_DIR = join(UserSettings.getUserHome(), '.cache/sourcify/public');
export const EDITOR_UI_DIST_DIR = join(dirname(require.resolve('sourcify-editor-ui')), 'dist');

export function getsourcifyPackageJson() {
	return jsonParse<sourcify.PackageJson>(readFileSync(join(CLI_DIR, 'package.json'), 'utf8'));
}

export const START_NODES = ['sourcify-nodes-base.start', 'sourcify-nodes-base.manualTrigger'];

export const sourcify_VERSION = getsourcifyPackageJson().version;

export const NODE_PACKAGE_PREFIX = 'sourcify-nodes-';
...
export const AUTH_COOKIE_NAME = 'sourcify-auth';
...
export const CREDENTIAL_BLANKING_VALUE = '__sourcify_BLANK_VALUE_e5362baf-c777-4d57-a609-6eaf1f9e87f6';
