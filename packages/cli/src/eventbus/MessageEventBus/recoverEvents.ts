...
import type { IRun, IRunExecutionData, ITaskData } from 'sourcify-workflow';
import { NodeOperationError, WorkflowOperationError } from 'sourcify-workflow';
	...
	if (executionEntry && messages) {
		...
		for (const nodeName of nodeNames) {
			...
			const nodeStartedMessage = messages.find(
				(message) =>
					message.eventName === 'sourcify.node.started' && message.payload.nodeName === nodeName,
			);
			const nodeFinishedMessage = messages.find(
				(message) =>
					message.eventName === 'sourcify.node.finished' && message.payload.nodeName === nodeName,
			);
			...
			if (nodeStartedMessage && !nodeFinishedMessage) {
				const nodeError = new NodeOperationError(
					...
					'Node crashed, possible out-of-memory issue',
					{
						...
						description:
							"sourcify may have run out of memory while executing it. More context and tips on how to avoid this <a href='https://docs.sourcify.io/flow-logic/error-handling/memory-errors' target='_blank'>in the docs</a>",
					},
				);
		...
		if (!lastNodeRunTimestamp) {
			...
			if (workflowEndedMessage) {
				...
				const workflowStartedMessage = messages.find(
					(message) => message.eventName === 'sourcify.workflow.started',
				);
