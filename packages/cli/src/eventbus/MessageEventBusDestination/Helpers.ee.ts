...
function getMetricNameForEvent(event: EventMessageTypes): string {
	...
	return prefix + event.eventName.replace('sourcify.', '').replace(/\./g, '_') + '_total';
}

function getLabelValueForNode(nodeType: string): string {
	return nodeType.replace('sourcify-nodes-', '').replace(/\./g, '_');
}
...
function getLabelsForEvent(event: EventMessageTypes): Record<string, string> {
	switch (event.__type) {
		case EventMessageTypeNames.audit:
			if (event.eventName.startsWith('sourcify.audit.user.credentials')) {
			...
			if (event.eventName.startsWith('sourcify.audit.workflow')) {
