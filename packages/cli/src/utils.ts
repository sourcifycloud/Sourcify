...
import { CliWorkflowOperationError, SubworkflowOperationError } from 'sourcify-workflow';
import type { INode } from 'sourcify-workflow';
...
function findWorkflowStart(executionMode: 'integrated' | 'cli') {
	return function (nodes: INode[]) {
		const executeWorkflowTriggerNode = nodes.find(
			(node) => node.type === 'sourcify-nodes-base.executeWorkflowTrigger',
		);
