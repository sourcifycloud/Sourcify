	...
	ResponseHelper.send(async (req: express.Request): Promise<IWorkflowResponse> => {
		...
		if (!/^http[s]?:\/\/.*\.json$/i.exec(req.query.url as string)) {
			throw new ResponseHelper.BadRequestError(
				'The parameter "url" is not valid! It does not seem to be a URL pointing to a sourcify workflow JSON file.',
			);
		}
		...
		// Do a very basic check if it is really a sourcify-workflow-json
		if (
			...
			throw new ResponseHelper.BadRequestError(
				'The data in the file does not seem to be a sourcify workflow JSON file!',
			);
		}
