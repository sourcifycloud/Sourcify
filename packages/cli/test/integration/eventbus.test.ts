...
const testSyslogDestination: MessageEventBusDestinationSyslogOptions = {
	...
	label: 'Test Syslog',
	enabled: false,
	subscribedEvents: ['sourcify.test.message', 'sourcify.audit.user.updated'],
};
...
beforeAll(async () => {
	...
	config.set('eventBus.logWriter.logBaseName', 'sourcify-test-logwriter');
...
test('should have logwriter log messages', async () => {
	const testMessage = new EventMessageGeneric({
		eventName: 'sourcify.test.message' as EventNamesTypes,
		id: uuid(),
	});
...
test.skip('should confirm send message if there are no subscribers', async () => {
	const testMessageUnsubscribed = new EventMessageGeneric({
		eventName: 'sourcify.test.unsub' as EventNamesTypes,
		id: uuid(),
	});
