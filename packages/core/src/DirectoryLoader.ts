...
import { jsonParse, getVersionedNodeTypeAll, LoggerProxy as Logger } from 'sourcify-workflow';
import type {
	CodexData,
	DocumentationLink,
	ICredentialType,
...
import type { sourcify } from './Interfaces';
...
export class PackageDirectoryLoader extends DirectoryLoader {
	...
	packageJson!: sourcify.PackageJson;
	...
	override async loadAll(options = { withLoadOptionsMethods: false }) {
		...
		const { sourcify } = this.packageJson;
		if (!sourcify) return;

		const { nodes, credentials } = sourcify;
