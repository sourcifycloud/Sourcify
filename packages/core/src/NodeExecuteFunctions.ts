...
import type {
	...
	IHttpRequestOptions,
	IsourcifyHttpFullResponse,
	IsourcifyHttpResponse,
	INode,
...
async function parseRequestObject(requestObject: IDataObject) {
	...
	// We are not using sourcify's interface as it would
...
export async function proxyRequestToAxios(
	...
	if (process.env.sourcify_USE_DEPRECATED_REQUEST_LIB) {
...
function convertsourcifyRequestToAxios(sourcifyRequest: IHttpRequestOptions): AxiosRequestConfig {
	// Destructure properties with the same name first.
	const { headers, method, timeout, auth, proxy, url } = sourcifyRequest;
	...
	axiosRequest.params = sourcifyRequest.qs;

	if (sourcifyRequest.baseURL !== undefined) {
		axiosRequest.baseURL = sourcifyRequest.baseURL;
	}

	if (sourcifyRequest.disableFollowRedirect === true) {
		axiosRequest.maxRedirects = 0;
	}

	if (sourcifyRequest.encoding !== undefined) {
		axiosRequest.responseType = sourcifyRequest.encoding;
	}

	if (sourcifyRequest.skipSslCertificateValidation === true) {
		axiosRequest.httpsAgent = new Agent({
			rejectUnauthorized: false,
		});
	}

	if (sourcifyRequest.arrayFormat !== undefined) {
		axiosRequest.paramsSerializer = (params) => {
			return stringify(params, { arrayFormat: sourcifyRequest.arrayFormat });
		};
	}

	const { body } = sourcifyRequest;
	if (body) {
		...
		if (existingContentTypeHeaderKey === undefined) {
			...
			axiosRequest.data = new URLSearchParams(sourcifyRequest.body as Record<string, string>);
		}
	...
	// header and the json flag. Header should take precedence.
	if (!userAgentHeader) {
		axiosRequest.headers['User-Agent'] = 'sourcify';
	}

	if (sourcifyRequest.ignoreHttpStatusErrors) {
...
): Promise<IsourcifyHttpFullResponse | IsourcifyHttpResponse> {
	let axiosRequest = convertsourcifyRequestToAxios(requestOptions);
	if (
...
/**
 * Takes a buffer and converts it into the format sourcify uses. It encodes the binary data as
...
/**
 * Takes generic input data and brings it into the json format sourcify uses.
 *
...
/**
 * Takes generic input data and brings it into the new json, pairedItem format sourcify uses.
...
export async function getCredentials(
	...
	// all credentials
	const fullAccess = ['sourcify-nodes-base.httpRequest'].includes(node.type);
		...
		return {
			...
			getInputSourceData: (inputIndex = 0, inputName = 'main') => {
				if (executeData?.source === null) {
					// Should never happen as sourcify sets it automatically
