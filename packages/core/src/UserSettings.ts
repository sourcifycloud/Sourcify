...
import { deepCopy } from 'sourcify-workflow';
import {
...
export async function getUserSettings(
	...
	try {
		...
		throw new Error(
			`Error parsing sourcify-config file "${settingsPath}". It does not seem to be valid JSON.`,
		);
	}
...
export function getUserSettingsPath(): string {
	const sourcifyFolder = getUsersourcifyFolderPath();

	return path.join(sourcifyFolder, USER_SETTINGS_FILE_NAME);
}

/**
 * Returns the path to the sourcify folder in which all sourcify
 * related data gets saved
 *
 */
export function getUsersourcifyFolderPath(): string {
	return path.join(getUserHome(), USER_SETTINGS_SUBFOLDER);
}

/**
 * Returns the path to the sourcify user folder with the custom
 * extensions like nodes and credentials
 *
 */
export function getUsersourcifyFolderCustomExtensionPath(): string {
	return path.join(getUsersourcifyFolderPath(), EXTENSIONS_SUBDIRECTORY);
}

/**
 * Returns the path to the sourcify user folder with the nodes that
 * have been downloaded
 *
 */
export function getUsersourcifyFolderDownloadedNodesPath(): string {
	return path.join(getUsersourcifyFolderPath(), DOWNLOADED_NODES_SUBDIRECTORY);
}
