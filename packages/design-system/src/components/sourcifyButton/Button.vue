<template>
	...
	>
		<span :class="$style.icon" v-if="loading || icon">
			<sourcify-spinner v-if="loading" :size="size" />
			<sourcify-icon v-else-if="icon" :icon="icon" :size="size" />
		</span>
...
import sourcifySpinner from '../sourcifySpinner';

export default defineComponent({
	name: 'sourcify-button',
	props: {
		label: {
			type: String,
		},
