import sourcifyNodeCreatorNode from './NodeCreatorNode.vue';
import { StoryFn } from '@storybook/vue';

export default {
	title: 'Modules/Node Creator Node',
	component: sourcifyNodeCreatorNode,
};

const DefaultTemplate: StoryFn = (args, { argTypes }) => ({
	props: Object.keys(argTypes),
	components: {
		sourcifyNodeCreatorNode,
	},
	template: `
		<sourcify-node-creator-node v-bind="$props">
			<template v-slot:icon>
				<img src="https://dev.w3.org/SVG/tools/svgweb/samples/svg-files/cartman.svg" />
			</template>
		</sourcify-node-creator-node>
	`,
});
...
const PanelTemplate: StoryFn = (args, { argTypes }) => ({
	...
	template: `
		<sourcify-node-creator-node v-bind="$props" :isPanelActive="isPanelActive" @click.capture="isPanelActive = true">
			<template v-slot:icon>
				<img src="https://dev.w3.org/SVG/tools/svgweb/samples/svg-files/cartman.svg" />
			</template>
