...
const SlotTemplate: StoryFn = (args, { argTypes }) => ({
	...
	template:
		'<sourcify-notice v-bind="$props">This is a notice! Thread carefully from this point forward.</sourcify-notice>',
});
...
const PropTemplate: StoryFn = (args, { argTypes }) => ({
	...
	template: '<sourcify-notice v-bind="$props"/>',
});
