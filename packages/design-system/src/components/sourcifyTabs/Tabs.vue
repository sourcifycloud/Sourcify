<template>
	<div :class="['sourcify-tabs', $style.container]">
		<div :class="$style.back" v-if="scrollPosition > 0" @click="scrollLeft">
			<sourcify-icon icon="chevron-left" size="small" />
		</div>
		<div :class="$style.next" v-if="canScrollRight" @click="scrollRight">
			<sourcify-icon icon="chevron-right" size="small" />
		</div>
		<div ref="tabs" :class="$style.tabs">
			...
			>
				<sourcify-tooltip :disabled="!option.tooltip" placement="bottom">
					<template #content>
					...
					>
						<div>
							...
							<span :class="$style.external"
								><sourcify-icon icon="external-link-alt" size="small"
							/></span>
						</div>
					</a>
					...
					<div
						...
						@click="() => handleTabClick(option.value)"
					>
						<sourcify-icon v-if="option.icon" :icon="option.icon" size="medium" />
...
export interface sourcifyTabOptions {
	value: string;
	label?: string;
	icon?: string;
	href?: string;
...
export default defineComponent({
	...
	props: {
		value: {},
		options: {
			type: Array as PropType<sourcifyTabOptions[]>,
			default: (): sourcifyTabOptions[] => [],
		},
	},
	methods: {
