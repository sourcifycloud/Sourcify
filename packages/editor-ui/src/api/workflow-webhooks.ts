...
const sourcify_API_BASE_URL = 'https://api.sourcify.io/api';
...
export async function fetchNextOnboardingPrompt(
	...
	return await get(sourcify_API_BASE_URL, ONBOARDING_PROMPTS_ENDPOINT, {
...
export async function applyForOnboardingCall(
	...
	try {
		const response = await post(sourcify_API_BASE_URL, ONBOARDING_PROMPTS_ENDPOINT, {
			instance_id: instanceId,
			user_id: `${instanceId}#${currentUer.id}`,
			email,
		});
...
export async function submitEmailOnSignup(
	...
	return await post(sourcify_API_BASE_URL, CONTACT_EMAIL_SUBMISSION_ENDPOINT, {
		instance_id: instanceId,
		user_id: `${instanceId}#${currentUer.id}`,
		email,
		agree,
