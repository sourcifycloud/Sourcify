		...
		lintSource(editorView: EditorView): Diagnostic[] {
			...
			try {
				...
				try {
					...
					/**
					 * For invalid (e.g. half-written) sourcify syntax, esprima errors with an off-by-one line number for the final line. In future, we should add full linting for sourcify syntax before parsing JS.
					 */
					return [];
				}
			}
			...
			if (this.mode === 'runOnceForAllItems') {
				...
				const isForOfStatementOversourcifyVar = (node: Node) =>
					...
					node.right.callee.object.name.startsWith('$'); // sourcify var, e.g $input

				const found = walk<TargetNode>(ast, isForOfStatementOversourcifyVar);
