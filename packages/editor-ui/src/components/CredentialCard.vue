<template>
	<sourcify-card :class="$style['card-link']" @click="onClick">
		<template #prepend>
			<credential-icon :credential-type-name="credentialType ? credentialType.name : ''" />
		</template>
		<template #header>
			<sourcify-heading tag="h2" bold class="ph-no-capture" :class="$style['card-heading']">
				{{ data.name }}
			</sourcify-heading>
		</template>
		...
		<template #append>
			<div :class="$style['card-actions']">
				<enterprise-edition :features="[EnterpriseEditionFeature.Sharing]">
					<sourcify-badge v-if="credentialPermissions.isOwner" class="mr-xs" theme="tertiary" bold>
...
import { ICredentialType } from 'sourcify-workflow';
