<template>
	<div :class="$style.container" data-test-id="node-credentials-config-container">
		...
		<template v-if="credentialPermissions.updateConnection">
			<sourcify-notice v-if="documentationUrl && credentialProperties.length" theme="warning">
				{{ $locale.baseText('credentialEdit.credentialConfig.needHelpFillingOutTheseFields') }}
				<span class="ml-4xs">
					<sourcify-link :to="documentationUrl" size="small" bold @click="onDocumentationUrlClick">
		...
		<sourcify-text v-if="isMissingCredentials" color="text-base" size="medium">
...
<script lang="ts">
import { ICredentialType, INodeTypeDescription } from 'sourcify-workflow';
...
export default mixins(restApi).extend({
	...
	async beforeMount() {
		...
		const key = `sourcify-nodes-base.credentials.${this.credentialType.name}`;
	...
	computed: {
		...
		documentationUrl(): string {
			...
			if (url.hostname === DOCS_DOMAIN) {
				url.searchParams.set('utm_source', 'sourcify_app');
