	...
	<div :class="$style.execListWrapper">
		<div :class="$style.execList">
			<div :class="$style.execListHeader">
				<sourcify-heading tag="h1" size="2xlarge">{{ this.pageTitle }}</sourcify-heading>
...
import type { IExecutionsSummary, ExecutionStatus } from 'sourcify-workflow';
...
export default mixins(externalHooks, genericHelpers, executionHelpers, restApi, showMessage).extend(
	{
		...
		mounted() {
			setPageTitle(`sourcify - ${this.pageTitle}`);
		},
		async created() {
