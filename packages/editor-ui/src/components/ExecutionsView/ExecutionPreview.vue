<template>
	<div v-if="executionUIDetails?.name === 'running'" :class="$style.runningInfo">
		<div :class="$style.spinner">
			<sourcify-spinner type="ring" />
		</div>
		<sourcify-text :class="$style.runningMessage" color="text-light">
			{{ $locale.baseText('executionDetails.runningMessage') }}
		</sourcify-text>
		<sourcify-button class="mt-l" type="tertiary" size="medium" @click="handleStopClick">
	...
	<div v-else :class="$style.previewContainer">
		...
		>
			<div>
				<sourcify-text size="large" color="text-base" :bold="true" data-test-id="execution-time">{{
				...
				<sourcify-text v-if="executionUIDetails.name === 'running'" color="text-base" size="medium">
					{{
				...
				<br /><sourcify-text v-if="activeExecution.mode === 'retry'" color="text-base" size="medium">
...
import { IAbstractEventMessage } from 'sourcify-workflow';
