<template>
	...
	>
		...
		></div>
		<sourcify-menu :items="mainMenuItems" :collapsed="isCollapsed" @select="handleSelect">
			<template #header>
				<div :class="$style.logo">
					<img
						:src="basePath + (isCollapsed ? 'sourcify-logo-collapsed.svg' : 'sourcify-logo-expanded.svg')"
						:class="$style.icon"
						alt="sourcify"
					/>
				</div>
			...
			<template #footer v-if="showUserArea">
				<div :class="$style.userArea">
					...
					>
						<sourcify-text size="small" :bold="true" color="text-dark">{{
							usersStore.currentUser.fullName
						}}</sourcify-text>
					</div>
					<div :class="{ [$style.userActions]: true, [$style.expanded]: fullyExpanded }">
						<sourcify-action-dropdown
							:items="userMenuItems"
							placement="top-start"
				...
				},
				{
					...
					children: [
						...
						},
						{
							...
							type: 'link',
							properties: {
								href: 'https://community.sourcify.io',
								newWindow: true,
							},
						},
						{
						...
						},
						{
							id: 'about',
							icon: 'info',
							label: this.$locale.baseText('mainSidebar.aboutsourcify'),
							position: 'bottom',
						},
					],
				},
