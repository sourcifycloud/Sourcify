			...
			<template #noResults>
				...
				>
					...
					<template v-else #action>
						...
						<sourcify-link v-if="[REGULAR_NODE_FILTER].includes(selectedView)" @click="addHttpNode">
							{{ $locale.baseText('nodeCreator.noResults.httpRequest') }}
						</sourcify-link>

						<sourcify-link v-if="[TRIGGER_NODE_FILTER].includes(selectedView)" @click="addWebHookNode()">
