<template>
	...
	>
		...
		<template #node-not-run>
			<sourcify-text v-if="workflowRunning && !isTriggerNode" data-test-id="ndv-output-waiting">{{
				$locale.baseText('ndv.output.waitingToRun')
			}}</sourcify-text>
			<sourcify-text v-if="!workflowRunning" data-test-id="ndv-output-run-node-hint">
				{{ $locale.baseText('ndv.output.runNodeHint') }}
				<span @click="insertTestData" v-if="canPinData">
					<br />
					{{ $locale.baseText('generic.or') }}
					<sourcify-text tag="a" size="medium" color="primary">
		...
		<template #no-output-data>
			<sourcify-text :bold="true" color="text-dark" size="large">{{
		...
		<template #recovered-artifical-output-data>
			<div :class="$style.recoveredOutputData">
				<sourcify-text tag="div" :bold="true" color="text-dark" size="large">{{
...
import { INodeTypeDescription, IRunData, IRunExecutionData, ITaskData } from 'sourcify-workflow';
import Vue from 'vue';
