<template>
	<Modal
		...
		:title="$locale.baseText('personalizationModal.customizesourcify')"
...
import { GenericValue } from 'sourcify-workflow';
		...
		survey() {
			const survey: IFormInputs = [
				...
				},
				{
					...
					properties: {
						...
						options: [
							...
							{
								label: this.$locale.baseText('personalizationModal.imNotUsingsourcifyForWork'),
				...
				},
				{
					...
					properties: {
						type: 'select',
						label: this.$locale.baseText('personalizationModal.howDidYouHearAboutsourcify'),
						placeholder: this.$locale.baseText('personalizationModal.select'),
						options: [
							{
								label: 'Google',
		...
		async onSubmit(values: IPersonalizationLatestVersion): Promise<void> {
			...
			try {
				const survey: Record<string, GenericValue> = {
					...
					personalization_survey_sourcify_version: this.rootStore.versionCli,
				};
