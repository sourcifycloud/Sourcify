<template>
	<div :class="$style.container">
				...
				>
					<template #content v-if="!isControlledPinDataTooltip">
						<div :class="$style.tooltipContainer">
							...
							<sourcify-text size="small" tag="p">
								{{ $locale.baseText('ndv.pinData.pin.description') }}

								<sourcify-link :to="dataPinningDocsUrl" size="small">
		...
		<div :class="$style.runSelector" v-if="maxRunIndex > 0" v-show="!editMode.enabled">
			...
			>
				<sourcify-icon-button
					...
					@click="unlinkRun"
				/>
				<sourcify-icon-button v-else icon="link" text type="tertiary" size="small" @click="linkRun" />
			</sourcify-tooltip>
		...
		<div v-if="maxOutputIndex > 0 && branches.length > 1" :class="$style.tabs">
			<sourcify-tabs :value="currentOutputIndex" @input="onBranchChange" :options="branches" />
		</div>
		...
		<div :class="$style.dataContainer" ref="dataContainer" data-test-id="ndv-data-container">
			<div v-if="isExecuting" :class="$style.center" data-test-id="ndv-executing">
				<div :class="$style.spinner"><sourcify-spinner type="ring" /></div>
				<sourcify-text>{{ executingMessage }}</sourcify-text>
			</div>
			...
			<div v-else-if="editMode.enabled" :class="$style.editMode">
				...
				<div :class="$style.editModeFooter">
					<sourcify-info-tip :bold="false" :class="$style.editModeFooterInfotip">
						{{ $locale.baseText('runData.editor.copyDataInfo') }}
						<sourcify-link :to="dataEditingDocsUrl" size="small">
			...
			<div v-else-if="paneType === 'input' && node.disabled" :class="$style.center">
				<sourcify-text>
					...
					<sourcify-link @click="enableNode">
			...
			<div v-else-if="hasNodeRun && hasRunError" :class="$style.stretchVertically">
				<sourcify-text v-if="isPaneTypeInput" :class="$style.center" size="large" tag="p" bold>
					{{
			...
			<div v-else-if="hasNodeRun && !showData" :class="$style.center">
				<sourcify-text :bold="true" color="text-dark" size="large">{{ tooMuchDataTitle }}</sourcify-text>
				<sourcify-text align="center" tag="div"
					><span
						v-html="
						...
						>
							<div>
								...
								<div v-if="binaryData.fileName">
									<div>
										<sourcify-text size="small" :bold="true"
								...
								<div v-if="binaryData.fileExtension">
									<div>
										<sourcify-text size="small" :bold="true"
											>{{ $locale.baseText('runData.fileExtension') }}:</sourcify-text
										>
									</div>
		...
		>
			...
			<div :class="$style.pageSizeSelector">
				<sourcify-select size="mini" :value="pageSize" @input="onPageSizeChange" popper-append-to-body>
					<template #prepend>{{ $locale.baseText('ndv.output.pageSize') }}</template>
					<sourcify-option v-for="size in pageSizes" :key="size" :label="size" :value="size">
					</sourcify-option>
					<sourcify-option :label="$locale.baseText('ndv.output.all')" :value="dataCount"> </sourcify-option>
				</sourcify-select>
			</div>
		</div>
		<sourcify-block-ui :show="blockUI" :class="$style.uiBlocker" />
	</div>
</template>
