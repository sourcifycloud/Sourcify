	...
	<div :class="$style.dataDisplay">
		<table :class="$style.table" v-if="tableData.columns && tableData.columns.length === 0">
			...
			>
				...
				>
					<sourcify-info-tip>{{ $locale.baseText('runData.emptyItemHint') }}</sourcify-info-tip>
				</td>
			...
			<thead>
				<tr>
					<th v-for="(column, i) in tableData.columns || []" :key="column">
						<sourcify-tooltip placement="bottom-start" :disabled="!mappingEnabled" :open-delay="1000">
							<template #content>
								<div>
				...
				<template>
					...
					>
						...
						>
							...
							>
							<sourcify-tree :nodeClass="$style.nodeClass" v-else :value="data">
...
import { GenericValue, IDataObject, INodeExecutionData } from 'sourcify-workflow';
