<template>
	<sourcify-card :class="$style.cardLink" data-test-id="destination-card" @click="onClick">
		<template #header>
			<div>
				<sourcify-heading tag="h2" bold class="ph-no-capture" :class="$style.cardHeading">
					{{ destination.label }}
				</sourcify-heading>
				<div :class="$style.cardDescription">
					<sourcify-text color="text-light" size="small">
		...
		<template #append>
			<div :class="$style.cardActions">
				<div :class="$style.activeStatusText" data-test-id="destination-activator-status">
					<sourcify-text v-if="nodeParameters.enabled" :color="'success'" size="small" bold>
						{{ $locale.baseText('workflowActivator.active') }}
					</sourcify-text>
					<sourcify-text v-else color="text-base" size="small" bold>
				...
				<sourcify-action-toggle :actions="actions" theme="dark" @action="onAction" />
			</div>
		</template>
	</sourcify-card>
</template>
