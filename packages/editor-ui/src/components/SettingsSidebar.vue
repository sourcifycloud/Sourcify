<template>
	<div :class="$style.container">
		<sourcify-menu :items="sidebarMenuItems" @select="handleSelect">
			<template #header>
				<div :class="$style.returnButton" @click="$emit('return')" data-test-id="settings-back">
					<i class="mr-xs">
						<font-awesome-icon icon="arrow-left" />
					</i>
					<sourcify-heading size="large" :bold="true">{{ $locale.baseText('settings') }}</sourcify-heading>
				</div>
			</template>
			<template #menuSuffix>
				<div :class="$style.versionContainer">
					<sourcify-link @click="onVersionClick" size="small">
		...
		sidebarMenuItems(): IMenuItem[] {
			const menuItems: IMenuItem[] = [
				...
				},
				{
					id: 'settings-api',
					icon: 'plug',
					label: this.$locale.baseText('settings.sourcifyapi'),
					position: 'top',
