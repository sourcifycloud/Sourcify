		...
		<transition name="fade" mode="out-in">
			...
			<div key="listening" v-else-if="isListeningForEvents">
				...
				<div v-if="isWebhookNode">
					<sourcify-text tag="div" size="large" color="text-dark" class="mb-2xs" bold>{{
			...
			<div key="default" v-else>
				...
				<div :class="$style.action">
					<div :class="$style.header">
						<sourcify-heading v-if="header" tag="h1" bold>
							{{ header }}
						</sourcify-heading>
						<sourcify-text v-if="subheader">
				...
				<sourcify-text size="small" @click="onLinkClick" v-if="activationHint">
				...
				<sourcify-link
					...
					>{{ $locale.baseText('ndv.trigger.moreInfo') }}</sourcify-link
				>
				<sourcify-info-accordion
					ref="help"
					v-if="executionsHelp"
