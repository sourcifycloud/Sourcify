	...
	>
		<template #header>
			<div :class="$style.title">
				<sourcify-heading tag="h2" size="medium" color="text-xlight">{{ getTitle }}</sourcify-heading>
			</div>
		</template>
		<template #content>
			<section :class="$style.content">
				<div v-if="showButtons" :class="$style.wrapper">
					...
					<div :class="$style.text">
						<sourcify-text size="small" color="text-xlight">Not likely</sourcify-text>
						<sourcify-text size="small" color="text-xlight">Very likely</sourcify-text>
					</div>
				</div>
				<div v-else :class="$style.email">
					...
					<div :class="$style.disclaimer">
						<sourcify-text size="small" color="text-xlight">
...
const DEFAULT_TITLE = 'How likely are you to recommend sourcify to a friend or colleague?';
const GREAT_FEEDBACK_TITLE =
	'Great to hear! Can we reach out to see how we can make sourcify even better for you?';
...
export default mixins(workflowHelpers).extend({
	...
	methods: {
		...
		async selectSurveyValue(value: string) {
			...
			const response: IsourcifyPromptResponse | undefined = await this.settingsStore.submitValueSurvey({
			...
			if (this.isEmailValid) {
				...
				if (response && response.updated) {
					...
					this.$showMessage({
						...
						message:
							'If you’d like to help even more, leave us a <a target="_blank" href="https://www.g2.com/products/sourcify/reviews/start">review on G2</a>.',
						type: 'success',
						duration: 15000,
					});
				}
