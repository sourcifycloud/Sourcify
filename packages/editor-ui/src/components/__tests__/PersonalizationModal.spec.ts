...
describe('PersonalizationModal.vue', () => {
	...
	it('should render correctly', async () => {
		...
		expect(wrapper.findAll('.sourcify-select').length).toEqual(5);
	...
	it('should display new option when role is "Devops", "Engineering", "IT", or "Sales and marketing"', async () => {
		...
		for (const index of [3, 4, 5, 6]) {
			wrapper.find('.sourcify-select[name="role"]').trigger('click');
			wrapper
				.find('.sourcify-select[name="role"]')
				.findAll('.el-select-dropdown__item')
				.at(index)
				.trigger('click');

			await retry(() => {
				expect(wrapper.findAll('.sourcify-select').length).toEqual(6);
				expect(wrapper.find('.sourcify-select[name^="automationGoal"]').exists()).toBe(true);
			});
		}
	});
});
