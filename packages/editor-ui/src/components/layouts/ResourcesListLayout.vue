<template>
	<page-view-layout>
		...
		<div v-if="loading">
			<sourcify-loading :class="[$style['header-loading'], 'mb-l']" variant="custom" />
			<sourcify-loading :class="[$style['card-loading'], 'mb-2xs']" variant="custom" />
			<sourcify-loading :class="$style['card-loading']" variant="custom" />
		</div>
		<template v-else>
			...
			<page-view-layout-list :overflow="type !== 'list'" v-else>
				<template #header>
					<div class="mb-xs">
						<div :class="$style['filters-row']">
							...
							<div :class="$style['sort-and-filter']">
								<sourcify-select v-model="sortBy" size="medium" data-test-id="resources-list-sort">
									<sourcify-option
					...
					<div v-show="hasFilters" class="mt-xs">
						<sourcify-info-tip :bold="false">
							{{ $locale.baseText(`${resourceKey}.filters.active`) }}
							<sourcify-link @click="resetFilters" size="small">
				...
				<sourcify-text color="text-base" size="medium" data-test-id="resources-list-empty" v-else>
					...
					<template v-if="shouldSwitchToAllSubview">
						<span v-if="!filters.search">
							...
							<sourcify-link @click="setOwnerSubview(false)">
								{{ $locale.baseText(`${resourceKey}.noResults.switchToShared.link`) }} </sourcify-link
							>)
						</span>
...
import { DatatableColumn } from 'sourcify-design-system';
