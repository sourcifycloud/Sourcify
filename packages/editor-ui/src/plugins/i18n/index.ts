...
import { locale } from 'sourcify-design-system';

import englishBaseText from './locales/en.json';
import { useUIStore } from '@/stores/ui';
import { useNDVStore } from '@/stores/ndv';
import { INodeProperties, INodePropertyCollection, INodePropertyOptions } from 'sourcify-workflow';
...
export class I18nClass {
	...
	shortNodeType(longNodeType: string) {
		return longNodeType.replace('sourcify-nodes-base.', '');
	}
	...
	credText() {
		...
		const credentialPrefix = `sourcify-nodes-base.credentials.${credentialType}`;
		const context = this;
	...
	nodeText() {
		...
		const initialKey = `sourcify-nodes-base.nodes.${nodeType}.nodeView`;
		const context = this;
		...
		return {
			...
			eventTriggerDescription(nodeType: string, eventTriggerDescription: string) {
				return context.dynamicRender({
					key: `sourcify-nodes-base.nodes.${nodeType}.nodeView.eventTriggerDescription`,
...
function setLanguage(language: string) {
	...
	// update sourcify design system and element ui
	locale.use(language);
...
export function addNodeTranslation(
	...
	const oldNodesBase = i18nInstance.messages[language]['sourcify-nodes-base'] || {};
	...
	const newNodesBase = {
		'sourcify-nodes-base': Object.assign(oldNodesBase, { nodes: updatedNodes }),
	};
...
export function addCredentialTranslation(
	...
	const newNodesBase = {
		'sourcify-nodes-base': Object.assign(oldNodesBase, { credentials: updatedCredentials }),
	};
