...
import { IDataObject, ITelemetrySettings } from 'sourcify-workflow';
...
export const useSettingsStore = defineStore(STORES.SETTINGS, {
	state: (): ISettingsState => ({
		settings: {} as IsourcifyUISettings,
		promptsData: {} as IsourcifyPrompts,
		userManagement: {
			enabled: false,
	...
	getters: {
		...
		pushBackend(): IsourcifyUISettings['pushBackend'] {
	...
	actions: {
		setSettings(settings: IsourcifyUISettings): void {
		...
		async getSettings(): Promise<void> {
			...
			rootStore.setsourcifyMetadata(settings.sourcifyMetadata || {});
		...
		setPromptsData(promptsData: IsourcifyPrompts): void {
		...
		async fetchPromptsData(): Promise<void> {
			...
			try {
				...
				const promptsData: IsourcifyPrompts = await getPromptsData(
		...
		async submitContactInfo(email: string): Promise<IsourcifyPromptResponse | undefined> {
			try {
				const usersStore = useUsersStore();
		...
		async submitValueSurvey(params: IsourcifyValueSurveyData): Promise<IsourcifyPromptResponse | undefined> {
			try {
				const usersStore = useUsersStore();
