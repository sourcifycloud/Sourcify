...
import {
	sourcifyPlusEndpoint,
	sourcifyPlusEndpointType,
	EVENT_PLUS_ENDPOINT_CLICK,
} from '@/plugins/endpoints/sourcifyPlusEndpointType';
	...
	methods: {
		...
		async openExecution(executionId: string) {
			...
			if ((data as IExecutionsSummary).waitTill) {
				this.$showMessage({
					...
					)}.<br/> <a href="https://docs.sourcify.io/integrations/builtin/core-nodes/sourcify-nodes-base.wait/" target="_blank">${this.$locale.baseText(
						'nodeView.moreInfo',
					)}</a>`,
					type: 'warning',
					duration: 0,
		...
		onConnectionDragAbortDetached(connection: Connection) {
			Object.values(this.instance?.endpointsByElement)
				...
				.filter((endpoint) => endpoint.endpoint.type === 'sourcifyPlus')
		...
		unbindCanvasEvents() {
			...
			for (const element of Object.values(elements)) {
				...
				for (const endpoint of endpoints || []) {
					...
					if (endpointInstance && endpointInstance.type === sourcifyPlusEndpointType) {
						(endpointInstance as sourcifyPlusEndpoint).unbindEvents();
					}
				}
			}
		},
		...
		getPlusEndpoint(nodeName: string, outputIndex: number): Endpoint | undefined {
			...
			return endpoints.find(
				(endpoint: Endpoint) =>
					// @ts-ignore
					endpoint.endpoint.type === 'sourcifyPlus' && endpoint?.__meta?.index === outputIndex,
			);
		},
		...
		onNodeRun({
			...
			if (data === null || data.length === 0 || waiting) {
				...
				endpoints.forEach((endpoint: Endpoint) => {
					if (endpoint.endpoint.type === 'sourcifyPlus') {
						(endpoint.endpoint as sourcifyPlusEndpoint).clearSuccessOutput();
					}
				});
						...
						(targetInputIndex: string) => {
							...
							if (endpoint && endpoint.endpoint) {
								...
								if (output && output.total > 0) {
									(endpoint.endpoint as sourcifyPlusEndpoint).setSuccessOutput(
			...
			try {
				...
				if (window.parent) {
					window.parent.postMessage(
						JSON.stringify({ command: 'sourcifyReady', version: this.rootStore.versionCli }),
						'*',
					);
				}
			} catch (error) {
