<template>
	<div :class="$style.container">
		...
		<div v-if="apiKey">
			<p class="mb-s">
				<sourcify-info-tip :bold="false">
					<i18n path="settings.api.view.info" tag="span">
						<template #apiAction>
							<a
								href="https://docs.sourcify.io/api"
								target="_blank"
						...
						<template #webhookAction>
							<a
								href="https://docs.sourcify.io/integrations/core-nodes/sourcify-nodes-base.webhook/"
								target="_blank"
			...
			<sourcify-card class="mb-4xs" :class="$style.card">
				<span :class="$style.delete">
					<sourcify-link @click="showDeleteModal" :bold="true">
			...
			<div :class="$style.hint">
				<sourcify-text size="small">
					{{
						$locale.baseText(`settings.api.view.${swaggerUIEnabled ? 'tryapi' : 'more-details'}`)
					}}
				</sourcify-text>
				<sourcify-link :to="apiDocsURL" :newWindow="true" size="small">
					{{
						$locale.baseText(
