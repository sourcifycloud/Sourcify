<template>
	<div v-if="!isLDAPFeatureEnabled">
		...
		<sourcify-info-tip type="note" theme="info-light" tooltipPlacement="right">
			<div>
	...
	<div v-else>
		...
		<div v-if="loginEnabled">
			<sourcify-heading tag="h1" class="mb-xl mt-3xl" size="medium">{{
...
import { sourcify_CONTACT_EMAIL, sourcify_SALES_EMAIL } from '@/constants';
...
export default mixins(showMessage).extend({
	...
	methods: {
		onContactUsClick(event: MouseEvent): void {
			const email = this.settingsStore.isCloudDeployment ? sourcify_CONTACT_EMAIL : sourcify_SALES_EMAIL;
