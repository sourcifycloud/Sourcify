<template>
	<div :class="$style.container" data-test-id="personal-settings-container">
		<div :class="$style.header">
			...
			<div class="ph-no-capture" :class="$style.user">
				<span :class="$style.username" data-test-id="current-user-name">
					<sourcify-text color="text-light">{{ currentUser.fullName }}</sourcify-text>
				</span>
				<sourcify-avatar
		...
		<div v-if="!signInWithLdap">
			<div :class="$style.sectionHeader">
				<sourcify-heading size="large">{{ $locale.baseText('settings.personal.security') }}</sourcify-heading>
			</div>
			<div>
				<sourcify-input-label :label="$locale.baseText('auth.password')">
					<sourcify-link @click="openPasswordModal" data-test-id="change-password-link">{{
