...
import { sourcify_PRICING_PAGE_URL } from '@/constants';
...
const openPricingPage = () => {
	...
	window.open(sourcify_PRICING_PAGE_URL, '_blank');
};
</script>

<template>
	<div>
		<sourcify-heading size="2xlarge">{{ locale.baseText('settings.usageAndPlan.title') }}</sourcify-heading>
		<sourcify-action-box
		...
		<div v-if="!usageStore.isDesktop && !usageStore.isLoading">
			<sourcify-heading :class="$style.title" size="large">
			...
			<div :class="$style.quota">
				<sourcify-text size="medium" color="text-light">
			...
			<div :class="$style.buttons">
				...
				<sourcify-button v-if="usageStore.managementToken" @click="onManagePlan" size="large">
					<a :href="managePlanUrl" target="_blank">{{
						locale.baseText('settings.usageAndPlan.button.manage')
					}}</a>
				</sourcify-button>
				<sourcify-button v-else @click="onViewPlans" size="large">
			...
			>
				...
				<template #footer>
					<sourcify-button @click="activationKeyModal = false" size="medium" type="secondary">
						{{ locale.baseText('settings.usageAndPlan.dialog.activation.cancel') }}
					</sourcify-button>
					<sourcify-button @click="onLicenseActivation" size="medium">
