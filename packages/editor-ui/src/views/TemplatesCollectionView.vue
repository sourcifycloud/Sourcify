		...
		<template #header>
			<div v-if="!notFoundError" :class="$style.wrapper">
				<div :class="$style.title">
					<sourcify-heading v-if="collection && collection.name" tag="h1" size="2xlarge">
						{{ collection.name }}
					</sourcify-heading>
					<sourcify-text v-if="collection && collection.name" color="text-base" size="small">
						{{ $locale.baseText('templates.collection') }}
					</sourcify-text>
					<sourcify-loading :loading="!collection || !collection.name" :rows="2" variant="h1" />
				</div>
			</div>
...
export default mixins(workflowHelpers).extend({
	...
	watch: {
		collection(collection: ITemplatesCollection) {
			if (collection) {
				setPageTitle(`sourcify - Template collection: ${collection.name}`);
			} else {
