		...
		<template #header>
			<div v-if="!notFoundError" :class="$style.wrapper">
				<div :class="$style.title">
					<sourcify-heading v-if="template && template.name" tag="h1" size="2xlarge">{{
						template.name
					}}</sourcify-heading>
					<sourcify-text v-if="template && template.name" color="text-base" size="small">
						{{ $locale.baseText('generic.workflow') }}
					</sourcify-text>
					<sourcify-loading :loading="!template || !template.name" :rows="2" variant="h1" />
				</div>
				<div :class="$style.button">
					...
					/>
					<sourcify-loading :loading="!template" :rows="1" variant="button" />
				</div>
			</div>
			<div :class="$style.notFound" v-else>
				<sourcify-text color="text-base">{{ $locale.baseText('templates.workflowsNotFound') }}</sourcify-text>
			</div>
		</template>
...
export default mixins(workflowHelpers).extend({
	...
	watch: {
		template(template: ITemplatesWorkflowFull) {
			if (template) {
				setPageTitle(`sourcify - Template template: ${template.name}`);
			} else {
				setPageTitle('sourcify - Templates');
			}
		},
	},
	async mounted() {
