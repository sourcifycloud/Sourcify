<template>
	...
	>
		<template #callout v-if="!hasActiveWorkflows">
			<sourcify-callout theme="secondary" icon="graduation-cap" class="mb-xs">
				{{ $locale.baseText('workflows.viewDemoNotice') }}

				<template #trailingContent>
					<sourcify-link size="small" theme="secondary" bold underline @click="goToTemplates">
		...
		<template #empty>
			<div class="text-center mt-s">
				<sourcify-heading tag="h2" size="xlarge" class="mb-2xs">
					{{
						$locale.baseText(
				...
				<sourcify-text size="large" color="text-base">
			...
			<div :class="['text-center', 'mt-2xl', $style.actionsContainer]">
				...
				>
					<sourcify-icon :class="$style.emptyStateCardIcon" icon="file" />
					<sourcify-text size="large" class="mt-xs" color="text-base">
				...
				>
					<sourcify-icon :class="$style.emptyStateCardIcon" icon="graduation-cap" />
					<sourcify-text size="large" class="mt-xs" color="text-base">
		...
		<template #filters="{ setKeyValue }">
			...
			<div class="mb-s">
				<sourcify-input-label
					...
					class="mb-3xs"
				/>
				<sourcify-select :value="filters.status" @input="setKeyValue('status', $event)" size="medium">
					<sourcify-option
