...
const sourcifyChunks = ['sourcify-workflow', 'sourcify-design-system'];
const ignoreChunks = [
	'vue2-boring-avatars',
...
function renderChunks() {
	...
	Object.keys(dependencies).forEach((key) => {
		if ([...vendorChunks, ...sourcifyChunks, ...ignoreChunks].includes(key)) {
			return;
		}
...
const alias = [
	...
	},
	{
		...
		replacement: path.resolve(
			__dirname,
			'..',
			'design-system/src/components/sourcifyButton/overrides/ElButton.ts',
		),
	},
];
...
export default mergeConfig(
	defineConfig({
		...
		css: {
			preprocessorOptions: {
				scss: {
					additionalData: '\n@use "@/sourcify-theme-variables.scss" as *;\n',
				},
			},
		},
		build: {
