...
to create credentials and nodes in TypeScript for sourcify.

```
npm install sourcify-node-dev -g
```
...
The commandline tool can be started with `sourcify-node-dev <COMMAND>`
...
sourcify custom extension folder (`~/.sourcify/custom/`) unless destination path is
...
The easiest way to create a new node is via the "sourcify-node-dev" cli. It sets up
all the basics.

A sourcify node is a JavaScript file (normally written in TypeScript) which describes
...
```TypeScript
import { IExecuteFunctions } from 'sourcify-core';
import {
	INodeExecutionData,
	INodeType,
...
- **name** [required]: Name of the node (for sourcify to use internally, in camelCase)
...
- **name** [required]: Name of the property (for sourcify to use internally, in camelCase)
