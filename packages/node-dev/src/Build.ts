...
export async function createCustomTsconfig() {
	...
	const tsconfigPath = join(dirname(require.resolve('sourcify-node-dev/src')), 'tsconfig-build.json');
...
export async function buildFiles({
	destinationFolder = UserSettings.getUsersourcifyFolderCustomExtensionPath(),
	watch,
	...
	// Supply a node base path so that it finds sourcify-core and sourcify-workflow
