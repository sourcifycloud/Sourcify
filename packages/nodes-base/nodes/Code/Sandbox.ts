...
import { isObject, REQUIRED_sourcify_ITEM_KEYS } from './utils';
...
export class Sandbox extends NodeVM {
	...
	async runCodeAllItems(): Promise<INodeExecutionData[]> {
		...
		if (Array.isArray(executionResult)) {
			/**
			 * If at least one top-level key is an sourcify item key (`json`, `binary`, etc.),
			 * then require all item keys to be an sourcify item key.
			 *
			 * If no top-level key is an sourcify key, then skip this check, allowing non-sourcify
			 * item keys to be wrapped in `json` when normalizing items below.
			 */
			const mustHaveTopLevelsourcifyKey = executionResult.some((item) =>
				Object.keys(item as IDataObject).find((key) => REQUIRED_sourcify_ITEM_KEYS.has(key)),
			);

			for (const item of executionResult) {
				if (mustHaveTopLevelsourcifyKey) {
	...
	private validateTopLevelKeys(item: INodeExecutionData) {
		Object.keys(item).forEach((key) => {
			if (REQUIRED_sourcify_ITEM_KEYS.has(key)) return;
			throw new ValidationError({
