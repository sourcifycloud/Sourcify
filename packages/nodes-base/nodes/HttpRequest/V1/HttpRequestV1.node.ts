// eslint-disable-next-line sourcify-nodes-base/node-filename-against-convention
import type {
	IExecuteFunctions,
	IDataObject,
	INodeExecutionData,
...
import { NodeApiError, NodeOperationError, sleep } from 'sourcify-workflow';
					...
					if (optionData.name === 'body' && parametersAreJson) {
						if (sendBinaryData) {
							...
							if (!contentTypesAllowed.includes(options.bodyContentType as string)) {
								// As sourcify-workflow.NodeHelpers.getParameterResolveOrder can not be changed
