...
import { BinaryDataManager, Credentials, constructExecutionMetaData } from 'sourcify-core';
import {
...
export function createTemporaryDir(prefix: string = 'sourcify') {
...
export function setup(testData: Array<WorkflowTestData> | WorkflowTestData) {
	...
	for (const nodeName of nodeNames) {
		if (!nodeName.startsWith('sourcify-nodes-base.')) {
			throw new Error(`Unknown node type: ${nodeName}`);
		}
		const loadInfo = knownNodes[nodeName.replace('sourcify-nodes-base.', '')];
		if (!loadInfo) {
			throw new Error(`Unknown node type: ${nodeName}`);
		}
