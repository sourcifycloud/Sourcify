...
export class RoutingNode {
	...
	async runPostReceiveAction(
		...
		responseData: IsourcifyHttpFullResponse,
	...
	async rawRoutingRequest(
		...
		credentialsDecrypted?: ICredentialsDecrypted,
	): Promise<IsourcifyHttpFullResponse> {
		let responseData: IsourcifyHttpFullResponse;
		...
		if (requestData.paginate && requestOperations?.pagination) {
			...
			if (typeof requestOperations.pagination === 'function') {
				...
				if (requestOperations.pagination.type === 'generic') {
					let tempResponseData: IsourcifyHttpFullResponse;
					...
					const additionalKeys = {
						...
						$response: {} as IsourcifyHttpFullResponse,
						$version: this.node.typeVersion,
					};
