...
const STICKY_NODE_TYPE = 'sourcify-nodes-base.stickyNote';
...
export function generateNodesGraph(
	...
	try {
		...
		otherNodes.forEach((node: INode, index: number) => {
			...
			if (node.type === 'sourcify-nodes-base.httpRequest' && node.typeVersion === 1) {
				try {
					nodeItem.domain = new URL(node.parameters.url as string).hostname;
				} catch {
					nodeItem.domain = getDomainBase(node.parameters.url as string);
				}
			} else if (node.type === 'sourcify-nodes-base.httpRequest' && [2, 3].includes(node.typeVersion)) {
			...
			} else if (node.type === 'sourcify-nodes-base.webhook') {
