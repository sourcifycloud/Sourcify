		...
		return new Proxy(
			{},
			{
				get(target, name, receiver) {
					...
					if (process.env.sourcify_BLOCK_ENV_ACCESS_IN_NODE === 'true') {
						throw new ExpressionError('access to env vars denied', {
							causeDetailed:
								'If you need access please contact the administrator to remove the environment variable ‘sourcify_BLOCK_ENV_ACCESS_IN_NODE‘',
							runIndex: that.runIndex,
							itemIndex: that.itemIndex,
							failExecution: true,
						});
		...
		const createExpressionError = (
			...
			if (context?.nodeCause) {
				...
				if (context.moreInfoLink && (pinData || isScriptingNode(nodeName, that.workflow))) {
					const moreInfoLink =
						' <a target="_blank" href="https://docs.sourcify.io/data/data-mapping/data-item-linking/item-linking-errors/">More info</a>';
